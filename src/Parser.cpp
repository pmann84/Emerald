...
std::optional<Node::Expr*> Parser::parse_expr(int min_precedence)
{
    ...
    while(true)
    {
        ...
        if (currentToken.value().is_binary_operator())
        {
            // Check for the operators binary precedence
            std::optional<uint8_t> precedence = currentToken.value().binary_precedence();
...
std::optional<Node::BinExpr*> Parser::parse_binary_expr(int min_precedence, Node::Expr* lhs)
{
    Token op = consume();
    auto exprRhs = parse_expr(min_precedence);
